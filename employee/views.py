...
def Employees(request):
    ...
    with connection.cursor() as cursor:
        cursor.execute("Select id,FirstName,LastName,Email,Gender,Phone,Dob,Age,City, count(1) over() TC From employee_employeelist ORDER BY id DESC LIMIT " +
                       str(ps) + " OFFSET "+str(ExcludeRecors))
...
def SingleEmployee(request, id):
    with connection.cursor() as cursor:
        cursor.execute(
            "Select * From employee_employeelist as EL INNER JOIN employee_skill as ES ON EL.user_id=ES.user_id Where id={}".format(id))
